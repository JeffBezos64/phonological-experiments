...
if os.path.exists('/csse/research/NativeLanguageID/mthesis-phonological/experiment/pickles/pickled_datasets/data_tokenize.pkl'):
    ...
    with open('/csse/research/NativeLanguageID/mthesis-phonological/experiment/pickles/pickled_datasets/data_tokenize.pkl', 'wb') as f:
    ...
    with open('/csse/research/NativeLanguageID/mthesis-phonological/experiment/pickles/pickled_datasets/data_spellcheck.pkl', 'wb') as f:
    ...
    with open('/csse/research/NativeLanguageID/mthesis-phonological/experiment/pickles/pickled_datasets/data_spellcheck_lemmatize.pkl', 'wb') as f:
    ...
    with open('/csse/research/NativeLanguageID/mthesis-phonological/experiment/pickles/pickled_datasets/data_spellcheck_stopwords.pkl', 'wb') as f:
    ...
    with open('/csse/research/NativeLanguageID/mthesis-phonological/experiment/pickles/pickled_datasets/data_lemmatize.pkl', 'wb') as f:
    ...
    with open('/csse/research/NativeLanguageID/mthesis-phonological/experiment/pickles/pickled_datasets/data_lemmatize_stopwords.pkl', 'wb') as f:
    ...
    with open('/csse/research/NativeLanguageID/mthesis-phonological/experiment/pickles/pickled_datasets/data_lemmatize_stopwords_spellcheck.pkl', 'wb') as f:
...
if process_parrish == True:
    ...
    for file in data_filenames:
            ...
            ParrishFeatureExtractorTrue = NonGenSimMeanTfidfEmbeddingVectorizer(ParrishEmbedderTrue)
            t2 = time.perf_counter()
            logger.info(f'ParrishEmbedderTrue {file} init time {t2 - t1}')
            transformed_data_matrix = ParrishFeatureExtractorTrue.fit_transform(data_df[file],labels)
    ...
    for file in data_filenames:
        ...
        transformed_data_matrix = ParrishFeatureExtractorFalse.fit_transform(data_df[file],labels)
        t3 = time.perf_counter()
        if record_performance_data == True:
            current, peak = tracemalloc.get_traced_memory()
            tracemalloc.stop()
...
if process_sharma == True:
    ...
    for file in data_filenames:  
        ...
        if record_performance_data == True:
            tracemalloc.start()
        t1 = time.perf_counter()
        SharmaFeatureExtractorTrue = NonGenSimMeanTfidfEmbeddingVectorizer(SharmaEmbedderTrue)
        t2 = time.perf_counter()
        logger.info(f'SharmaEmbedderTrue {file} init time {t2 - t1}')
        transformed_data_matrix = SharmaFeatureExtractorTrue.fit_transform(data_df[file],labels)
        t3 = time.perf_counter()
        if record_performance_data == True:
            current, peak = tracemalloc.get_traced_memory()
            tracemalloc.stop()
    ...
    for file in data_filenames:
        ...
        if record_performance_data == True:
            tracemalloc.start()
        t1 = time.perf_counter()
        SharmaFeatureExtractorFalse = NonGenSimMeanTfidfEmbeddingVectorizer(SharmaEmbedderFalse)
        t2 = time.perf_counter()
        logger.info(f'SharmaEmbedderFalse {file} init time {t2 - t1}')
        transformed_data_matrix = SharmaFeatureExtractorFalse.fit_transform(data_df[file],labels)
        t3 = time.perf_counter()
        if record_performance_data == True:
            current, peak = tracemalloc.get_traced_memory()
            tracemalloc.stop()
...
if process_zouhar == True:
    ...
    for file in data_filenames:
        ...
        if record_performance_data == True:
            tracemalloc.start()
        t1 = time.perf_counter() 
        ZouharFeatureExtractor = NonGenSimMeanTfidfEmbeddingVectorizer(ZouharEmbedder)
        t2 = time.perf_counter()
        logger.info(f'ZouharEmbedder {file} init time {t2 - t1}')
        transformed_data_matrix = ZouharFeatureExtractor.fit_transform(data_df[file],labels)
        t3 = time.perf_counter()
        if record_performance_data == True:
            current, peak = tracemalloc.get_traced_memory()
            tracemalloc.stop()
...
if process_glove == True:
    ...
    for file in data_filenames:
        ...
        if record_performance_data == True:
            tracemalloc.start()
        t1 = time.perf_counter() 
        GloveFeatureExtractorTrue = NonGenSimMeanTfidfEmbeddingVectorizer(GloveEmbedderTrue)
        t2 = time.perf_counter()
        logger.info(f'GloveEmbedderTrue {file} init time {t2 - t1}')
        transformed_data_matrix = GloveFeatureExtractorTrue.fit_transform(data_df[file],labels)
        t3 = time.perf_counter()
        if record_performance_data == True:
            current, peak = tracemalloc.get_traced_memory()
            tracemalloc.stop()
    ...
    for file in data_filenames:
        ...
        if record_performance_data == True:
            tracemalloc.start()
        t1 = time.perf_counter() 
        GloveFeatureExtractorFalse = NonGenSimMeanTfidfEmbeddingVectorizer(GloveEmbedderFalse)
        t2 = time.perf_counter()
        logger.info(f'GloveEmbedderFalse {file} init time {t2 - t1}')
        transformed_data_matrix = GloveFeatureExtractorFalse.fit_transform(data_df[file],labels)
        t3 = time.perf_counter()
        if record_performance_data == True:
            current, peak = tracemalloc.get_traced_memory()
            tracemalloc.stop()
...
if process_fasttext == True:
    ...
    for file in data_filenames:
        ...
        if record_performance_data == True:
            tracemalloc.start()
        t1 = time.perf_counter() 
        FastTextFeatureExtractorTrue = NonGenSimMeanTfidfEmbeddingVectorizer(FastTextEmbedderTrue)
        t2 = time.perf_counter()
        logger.info(f'FastTextEmbedderTrue {file} init time {t2 - t1}')
        transformed_data_matrix = FastTextFeatureExtractorTrue.fit_transform(data_df[file],labels)
        t3 = time.perf_counter()
        if record_performance_data == True:
            current, peak = tracemalloc.get_traced_memory()
            tracemalloc.stop()
    ...
    for file in data_filenames:
        ...
        if record_performance_data == True:
            tracemalloc.start()
        t1 = time.perf_counter() 
        FastTextFeatureExtractorFalse = NonGenSimMeanTfidfEmbeddingVectorizer(FastTextEmbedderFalse)
        t2 = time.perf_counter()
        logger.info(f'FastTextEmbedderFalse {file} init time {t2 - t1}')
        transformed_data_matrix = FastTextFeatureExtractorFalse.fit_transform(data_df[file],labels)
        t3 = time.perf_counter()
        if record_performance_data == True:
            current, peak = tracemalloc.get_traced_memory()
            tracemalloc.stop()
